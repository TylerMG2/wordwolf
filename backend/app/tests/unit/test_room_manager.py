...
async def test_room_broadcast():
    ...
    player.connect(MockWebsocket())
    player_2.connect(MockWebsocket())
    await room.broadcast(RoomEvent.ROOM_STATE, "test")

    # Check that both players received the message
    assert len(player.websocket.data) == 1
    assert len(player_2.websocket.data) == 1
    assert player.websocket.data[0].action == RoomEvent.ROOM_STATE
    assert player_2.websocket.data[0].action == RoomEvent.ROOM_STATE
...
async def test_room_broadcast_except():
    ...
    await room.broadcast_except(RoomEvent.ROOM_STATE, "test", player.player_id)
    ...
    assert player_2.websocket.data[0].action == RoomEvent.ROOM_STATE
    assert player_3.websocket.data[0].action == RoomEvent.ROOM_STATE
    assert player_2.websocket.data[0].data == "test"
